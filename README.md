...
5090 服务器上跑在 `0.0.0.0:8060` 的运维/演示 Web 服务（原目录 `~/da3-web`），单文件 FastAPI 应用，纯服务端渲染、零前端构建。（「5090」沿用主机称谓；2026-08-24 起双卡：**RTX Pro 6000 Blackwell 96GB（独占 vLLM）+ RTX 5090 32GB（SAM3/DA3）**，全文不改名。）
...
- **content 重排**：`[固定段] → 任务二清单 → 逐个「标签+参考图」→ 【当前画面】→ 带框图 → 收尾`。
...
> **模型单例 + GPU 共存**：进程内只加载一份 DA3 模型权重，并用一把 GPU 锁串行化推理。模型懒加载：启动不占显存，首次推理才加载一份（约 6.5GB，推理峰值约 8.6GB@process_res=504）。2026-08-24 起双卡布局：DA3 随 da3-web unit 的 `CUDA_VISIBLE_DEVICES` 钉在 **GPU1=RTX 5090 32GB**，与 SAM3（实占约 7G）同卡、与 GPU0 上独占的 vLLM 物理隔离；SAM3 上限 0.9 不为 DA3 预留（沿用「显存不足任其失败」纪律，常态互不影响）。`process_res` 仍是显存的主调节开关，极端情况下 OOM 时调低重试。
