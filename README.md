...
| `exp_app.py` | **感知链路实验台（8061）**：与产线 8060 完全隔离的「完整识别链路」实验环境，UI 复刻 8060 主页——左栏 设备帧/实验检测标注帧/点云产物(共享 8060 产线 DA3 产物)，右栏 完整 VLM 识别卡片流（与 /recog 同款 UI，prompt/解析/五道合并闸门移植自产线、可独立魔改）。「调节」抽屉热改 采样率/SAM3 食物·液体查询词/score 阈值/LA 对照/识别节流，留 embedding 匹配插槽（`_embed_match()`）。帧取自 8060 只读接口，SAM3 走无状态 `/v1/segment` 不碰产线流式，不加载新模型。手动起停：`nohup ./run-exp.sh > exp.log 2>&1 &` |
