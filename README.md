...
5090 服务器上跑在 `0.0.0.0:8060` 的运维/演示 Web 服务（原目录 `~/da3-web`），单文件 FastAPI 应用，纯服务端渲染、零前端构建。（「5090」沿用主机称谓；显卡已于 2026-08 换为 **RTX Pro 6000 Blackwell 96GB**，全文不改名。）
...
> **模型单例 + GPU 共存**：进程内只加载一份 DA3 模型权重，并用一把 GPU 锁串行化推理——显存与同机服务共享。模型懒加载：启动不占显存，首次推理才加载一份（约 6.5GB，推理峰值约 8.6GB@process_res=504）。换卡 RTX Pro 6000 96GB 后余量充足（本机 vLLM 约 64G + SAM3 约 9G 之外仍有富余），常态不再显存紧张；`process_res` 仍是显存的主调节开关，极端情况下 OOM 时调低重试。
