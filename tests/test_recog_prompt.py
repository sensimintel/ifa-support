...
没人会想到是 prompt 里多了一句「2 food box(es)」。
...
class FixedHeadTest(unittest.TestCase):
    ...
    def test_carries_no_per_round_values(self):
        ...
        for leak in ("food box(es)", "liquid box(es)", "the previous round displayed",
                     "CANDIDATE[1] 食物0", "recorded within the last 30 seconds"):
    ...
    def test_reference_catalog_section_only_when_present(self):
        ...
        self.assertIn("Task 0", with_refs)
        self.assertIn("REFERENCE badge", with_refs)
        self.assertIn("high", with_refs)
        self.assertNotIn("Task 0", recog_prompt.fixed_head(False))

    def test_bans_ordinal_references(self):
        # 旧版靠「图1/图2/图3」指称，一条消息里塞十几张图时根本对不齐
        head = recog_prompt.fixed_head(False)
        self.assertIn("never by ordinal position", head)
        self.assertNotIn("image 1 = the current frame", head)

    def test_grounding_contract_is_checkable(self):
        """「注意力放在当前画面」必须落成可检查的输出契约，而不是一句嘱咐。"""
        head = recog_prompt.fixed_head(False)
        self.assertIn("write it into the seen field", head)
        self.assertIn("**name must be supported by the facts written in seen**", head)
        # 现场那个 case：金黄包装 → 橙色 → Orange，必须被这条堵死
        self.assertIn("the colour of the packaging is not the colour of the content", head)

    def test_evidence_code_requires_collateral(self):
        head = recog_prompt.fixed_head(False)
        self.assertIn("cur_text is not none", head)   # B=1 的抵押物
        self.assertIn("? is nothing to be ashamed of, a made-up 1 is the mistake", head)
    ...
    def test_catalog_hit_example_only_when_refs(self):
        ...
        self.assertIn("Example 5", with_refs)        # 命中示例排第一，共 5 个
        no_refs = recog_prompt.fixed_head(False)
        self.assertNotIn("Milka", no_refs)
        self.assertNotIn("Example 5", no_refs)       # 无参考库只有 4 个
...
class EdibleConstraintTest(unittest.TestCase):
    ...
    def test_fixed_head_bans_non_food_items(self):
        ...
        self.assertIn("food or drink that can go into a mouth", p)
        self.assertIn("**Packaging alone is not food**", p)

    def test_fixed_head_asks_edible_field(self):
        p = recog_prompt.fixed_head(True)
        self.assertIn("edible: boolean true/false", p)
    ...
    def test_tail_repeats_the_edible_constraint(self):
        ...
        self.assertIn("food or drink that can go into a mouth", t)
        self.assertIn("items must be the empty array", t)
...
class VariableBlocksTest(unittest.TestCase):
    ...
    def test_candidate_numbering_matches_labels(self):
        ...
        for i, c in enumerate(cands):
            ...
            self.assertIn("CANDIDATE[%d] %s" % (i + 1, c["name"]),
    ...
    def test_intro_warns_names_may_be_wrong(self):
        ...
        self.assertIn("some of them may simply be wrong", intro)
        self.assertIn("not that it is still on the table", intro)

    def test_empty_candidates_branch(self):
        intro = recog_prompt.candidates_intro([])
        self.assertIn("match is always null", intro)
        self.assertIn("match_evidence is NONE", intro)

    def test_current_label_closes_the_reference_section(self):
        with_c = recog_prompt.current_label(True)
        self.assertIn("The [PAST REFERENCE] images end here.", with_c)
        self.assertIn("[CURRENT FRAME]", with_c)
        self.assertNotIn("end here", recog_prompt.current_label(False))

    def test_boxed_label_carries_the_counts(self):
        label = recog_prompt.boxed_label(2, 1)
        self.assertIn("2 food box(es)", label)
        self.assertIn("1 liquid box(es)", label)

    def test_tail_repeats_the_only_constraint_that_matters(self):
        tail = recog_prompt.tail(None, [])
        self.assertIn("describe only what really exists in that [CURRENT FRAME]", tail)
...
class PickRuleTest(unittest.TestCase):
    ...
    def test_no_history_degrades(self):
        self.assertIn("the one you are most confident about",
                      recog_prompt.pick_rule(None, []))
    ...
    def test_confirmation_bias_wording_gone(self):
        ...
        self.assertIn("list entry [1]", t)
        self.assertIn("not guaranteed to be correct", t)
        self.assertIn("write seen and name independently", t)
        self.assertNotIn("keep picking it", t)
        self.assertNotIn("still in the current frame", t)

    def test_prefers_switching_over_staying_wrong(self):
        t = recog_prompt.pick_rule({"name": "Orange", "card_id": 1}, [])
        self.assertIn("a name that stays wrong is far worse than a name that changes once", t)
