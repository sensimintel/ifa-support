...
**ABSENT 是「端起碗吃再放回」能被识别出来的全部机制**：一次取食 0.2–3 g，端起一个
碗几十到几百克，两者差两个数量级。跌幅越过离台阈就把离台前的平台压成锚点，等碗
回来时用「新平台 − 锚点」一次性结算这期间吃掉的总量——碗在手上那段秤读数没有意义，
状态机干脆不看。

**本模块只吐原始变化，不做分类**（是蓝莓还是爆米花、要不要抵消手扶碗那一对），
