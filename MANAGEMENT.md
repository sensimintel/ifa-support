...
> 文中「5090」沿用主机称谓；显卡已于 2026-08 换为 **RTX Pro 6000 Blackwell 96GB**（原 RTX 5090 32GB），不做全文改名。

| 部署点 | 状态 | 待办 |
|---|---|---|
| 5090 本机 VLM（RTX Pro 6000 96GB） | **VLM 已本机化承接**：宿主裸进程 `vllm serve :8000`（Qwen3.6-35B-A3B-FP8，别名 `gemini-3.1-pro-preview`，`gpu-memory-utilization 0.73` 约 64G）；services 的 `chunk_inhouse.base_url` 已切 `http://172.23.0.1:8000/v1`（2026-08-21 服务器手工生效，repo 配置另行同步）。显存现状：vLLM ~64G + SAM3 ~9G，余量充足 | repo 侧配置同步落库后复核一致 |
