...
本目录沉淀 ifa-support 演示栈依赖的 **SAM3 模型服务**的全部部署脚本与信息（原 DA3 部分已于 2026-08-25 随模型退役删除）。

**第一性目标**：在一台「稳定、有外网、显存足够」的新服务器上，凭本目录 + 本仓根目录，
一次性拉起所有模型、对应服务 gateway 与全部依赖，复刻 5090 现网的完整演示栈。
（「5090」沿用主机称谓；2026-08-24 起双卡：**RTX Pro 6000 Blackwell 96GB（独占 vLLM）+ RTX 5090 32GB（SAM3）**，不做全文改名。DA3 已于 2026-08-25 整体退役：`model/da3/` 删除，服务器上的 `~/Depth-Anything-3` 源码与权重一并清除。）

## 架构与端口

```
设备帧 → app.py(:8060, 本仓根目录, 帧中继 + 各页面 gateway，进程内无模型)
...
| 端口 | 服务 | 来源 |
|---|---|---|
| 8060 | ifa web（本仓 `app.py`，纯 gateway/中继） | 本仓根目录 `run.sh` / `da3-web.service` |
| 8013 | SAM3 | `model/sam3/` |

## 硬件 / 系统前置

- NVIDIA 驱动 ≥ 580（SAM3 环境用 torch cu130 需 CUDA 13。现网驱动版本以 `nvidia-smi` 实测为准——2026-08 换卡 RTX Pro 6000 后驱动已随卡更新，不要按旧记录写死）
- 外网：github.com、huggingface.co（国内可 `export HF_ENDPOINT=https://hf-mirror.com`；
  **例外：`facebook/sam3` 是 gated 仓，必须官方源 + `HF_TOKEN`**，镜像上没有）
- 磁盘：权重 sam3.pt 3.45GB + venv 若干 GB

## 显存预算（2026-08-24 起双卡现网口径）

双卡布局：GPU0 = RTX Pro 6000 Blackwell 96GB **独占给 vLLM**；GPU1 = RTX 5090 32GB 归 SAM3 独享（DA3 已退役）。三个 unit 都在 systemd 里钉卡，任何重启都各归各卡：sam3/da3-web 用 `CUDA_VISIBLE_DEVICES=<GPU UUID>`（torch 认 UUID；CUDA 默认枚举「最快卡优先」，裸 index 不稳定；da3-web 已不占 GPU，钉卡仅作兜底）；vllm 用 `CUDA_DEVICE_ORDER=PCI_BUS_ID` + `CUDA_VISIBLE_DEVICES=0`（**vLLM 按整数解析该变量、不认 UUID**——UUID 会 ValidationError 崩溃循环，2026-08-24 实测；PCI 序稳定，0=02:00.0=Pro 6000）。
...
GPU1 上仅 SAM3 常态 ~7G（DA3 已于 2026-08-25 退役），显存宽裕；SAM3 上限 0.9 是隔离兜底而非预留。旧「96G 单卡共享」与更早的「32G 紧张 / OOM 风险」口径均作废。

## 拉起顺序（新机器从零）

```bash
# 1. SAM3：venv + 权重(需 HF_TOKEN) + systemd 服务
model/sam3/setup.sh

# 2. web 本体：本仓根目录（先按新机器路径改 run.sh 里的 python 路径与 .env）
...
| web 运行时 | conda env `da3`(py3.10，环境名沿用历史称谓；`~/Depth-Anything-3` 已删除) |
