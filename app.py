...
function applyAdaptive(){  // 自适应：保留角度 θ/φ 与 FOV；每帧按 getDimensions 算距离、对准 getBoundingBoxCenter
  ...
  if(mv.jumpCameraToGoal)mv.jumpCameraToGoal();
  lastCam='camera-orbit="'+camState.theta+'deg '+camState.phi+'deg '+dist.toFixed(2)+'m" '
         +'field-of-view="'+camState.fov+'deg" '
         +'camera-target="'+c.x.toFixed(2)+'m '+c.y.toFixed(2)+'m '+c.z.toFixed(2)+'m"  （自适应：每帧跟随点云中心/尺度）';
  $('camnow').textContent=lastCam;
}
