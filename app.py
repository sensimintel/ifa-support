...
function lockFromNow(){  // 把「当前实测视角」固化为锁定视角，并刷新只读框
  try{
    const n=readNow();
    // radius 锁「百分比」而非米：点云米级尺度极小且逐帧不同，锁米值会算出 ~0 甚至负距离；
    // % 相对每帧包围盒，跨帧稳定。以滑块 camState.radius 为准（鼠标缩放改米不改%，远近用滑块调）。
    const rpct=camState.radius;
    locked={orbit:n.th.toFixed(2)+'deg '+n.ph.toFixed(2)+'deg '+rpct+'%',
            target:n.tx.toFixed(4)+'m '+n.ty.toFixed(4)+'m '+n.tz.toFixed(4)+'m',
            fov:n.fov.toFixed(2)+'deg'};
    lastCam='camera-orbit="'+n.th.toFixed(1)+'deg '+n.ph.toFixed(1)+'deg '+rpct+'%" '
