...
> 文中「5090」沿用主机称谓（2026-08-24 起双卡：GPU0=RTX Pro 6000 96GB 独占 vLLM，GPU1=RTX 5090 32GB 跑 SAM3+DA3，unit 按 UUID 钉卡）。改到 SAM3 相关链路请同步更新本文。
...
| 段 | 含义 | 归属 |
|---|---|---|
| `frame_age_ms` | 帧到达 8060 → **触发线程拿起它**（纯等待） | 推帧节奏 + 触发间隔 |
| `decode_ms` | JPEG 解码成 ndarray | 本机 CPU |
| `gate_ms` | SAM3 门控（直传口径无此段） | 本机 GPU + 同机 8013 调用 |
| `enqueue_ms` | 门控跑完 → 任务入队的零头 | 本机 CPU |
| `wait_ms` | 入队 → worker 取走 | 并发不足才明显 |
| `encode_ms` | 两张原图 JPEG q95 + base64 + JSON 序列化 | 本机 CPU |
| `http_ms` | 发请求 → 收完响应 | **网络往返（VLM 已本机化，基本可忽略）+ 服务端排队 + 推理 + 回传** |
| `parse_ms` | 解析输出 + guardrail | 本机 CPU |
| `post_ms` | 去重五闸 + 建卡/合并 | 本机 CPU |

⚠️ 帧龄的终点是「触发线程拿起帧」，**不是入队**：解码与 SAM3 门控都发生在入队之前，
用入队时刻算帧龄会把它们算两遍（实测各段之和比端到端多出 183ms，正好是 decode+gate）。

`http_ms` 在这一层拆不开网络与推理。日志里的 `tunnel_rtt_ms` **已失效、待移除**：它原是
「5090→Mac→IAP→GCP」隧道路径的网络基线探测（GET `8011/v1/models`），该链路已随 VLM
本机化整体废弃（识别直连本机 vLLM `:8000`），这条路径不复存在，不要再拿它归因。本机
直连下网络往返可忽略，`http_ms` 基本就是服务端排队与模型推理。`req_bytes` 给出本轮请求体
大小（图片 base64 占大头）；`prompt_tokens`/`completion_tokens`
取自响应的 `usage`——**vLLM 的 OpenAI 兼容响应不带服务端耗时字段**，token 数是
「这轮为什么慢」最可归因的一项（输出越多解码越久）。真要拿到纯推理时间只能改流式
请求测首 token，那会动生产请求形态，没做。
