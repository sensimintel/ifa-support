...
> 最后核对：2026-08-17（`c831ea5` + 直传识别改动）。改到 SAM3 相关链路请同步更新本文。
...
| B | 8060 双目 IR 链（`_stereo_sam3_overlays`） | 左目 IR 每词 `/v1/stream/frame`（**独立 session 表**，与 A 的记忆完全隔离） | ①双目点云染色 ②双目高亮点云 ③识别触发证据 | **在跑**（g335 推 aux 时） |
