...
# 设备硬件深度渲染回调：fn(depth_png16_bytes, aux_info) -> (渲染图字节, content_type)。
# 本模块零重依赖不碰 cv2，伪彩渲染由 app.py 注入；未注入时 /api/frame/aux-image
# 的 depth 档直接回原始 16bit PNG。
_depth_renderer: Optional[Callable[[bytes, dict], tuple]] = None
...
def set_depth_renderer(fn: Callable[[bytes, dict], tuple]) -> None:
    """注入设备硬件深度的伪彩渲染回调（纯 CPU，按 aux_seq 惰性渲染并缓存）。"""
...
def _new_bucket(now: float) -> dict:
    return {
        ...
        # 辅助帧（/api/frame/aux）：左右 IR JPEG + 硬件深度 16bit PNG + 随帧元信息。
        # aux_info 是解析后的 camera_info 字典（stereo_supported/baseline_mm 等）
        "aux_left": None, "aux_right": None, "aux_depth": None,
        "aux_seq": 0, "aux_received_at": 0.0, "aux_info": None,
        # 硬件深度伪彩渲染缓存（按 aux_seq 惰性渲染，见 /api/frame/aux-image）
        "aux_depth_render": None, "aux_depth_render_ct": "image/jpeg",
        "aux_depth_render_seq": 0,
...
async def ingest_aux_frame(
    ...
    """接收辅助帧（cam-pusher 与主帧同节拍推送）：左/右 IR 灰度 JPEG + 硬件深度
    16bit PNG，至少带其一。camera_info JSON 里除 device_id 外还带
    stereo_supported / baseline_mm / depth_scale_mm / laser_mode，解析后存 aux_info
    供双目处理与面板展示。左右 IR 必须成对（只来一只按缺失处理）。"""
    left_b = await left.read() if left is not None else None
    right_b = await right.read() if right is not None else None
    depth_b = await depth.read() if depth is not None else None
    if not (left_b and right_b) and not depth_b:
        return JSONResponse({"ok": False, "error": "辅助帧为空（需左右 IR 成对或深度）"},
                            status_code=400)
    dev = _parse_device_id(camera_info)
    try:
    ...
    with _cv:
        ...
        if depth_b:
            st["aux_depth"] = depth_b
        st["aux_received_at"] = now
        st["aux_info"] = info
        _cv.notify_all()
    return JSONResponse({"ok": True, "device": dev, "aux_seq": seq,
                         "stereo": bool(left_b and right_b), "depth": bool(depth_b)})


@router.get("/api/frame/aux-image")
def latest_aux_image(kind: str = Query(...), device: Optional[str] = Query(None)):
    """返回指定设备（缺省=选中设备）最新辅助帧图片：
      kind=left|right → IR 灰度 JPEG；
      kind=depth      → 硬件深度伪彩渲染图（渲染回调未注入时回原始 16bit PNG）。
    深度渲染按 aux_seq 惰性缓存：同一帧只渲一次，渲染在锁外做。"""
    if kind not in ("left", "right", "depth"):
    ...
    with _cv:
        ...
        if st["aux_depth_render_seq"] == seq and st["aux_depth_render"] is not None:
            return Response(content=st["aux_depth_render"],
                            media_type=st["aux_depth_render_ct"],
                            headers={"Cache-Control": "no-store",
                                     "X-Frame-Seq": str(seq)})
    renderer = _depth_renderer
    if renderer is None:
        return Response(content=raw, media_type="image/png",
                        headers={"Cache-Control": "no-store", "X-Frame-Seq": str(seq)})
    try:
        rendered, ct = renderer(raw, info)
    except Exception as e:
        return JSONResponse({"error": f"深度渲染失败：{type(e).__name__}: {e}"},
                            status_code=500)
    with _cv:
        st2 = _devices.get(_dev)
        # 渲染期间可能已有更新帧，只在 seq 未变时写缓存；返回本次渲染结果不受影响
        if st2 is not None and st2["aux_seq"] == seq:
            st2["aux_depth_render"] = rendered
            st2["aux_depth_render_ct"] = ct
            st2["aux_depth_render_seq"] = seq
    return Response(content=rendered, media_type=ct,
                    headers={"Cache-Control": "no-store", "X-Frame-Seq": str(seq)})
    ...
    with _cv:
        ...
        if st is None:
            return JSONResponse({
                ...
                "has_aux_stereo": False, "has_aux_depth": False, "aux_info": None,
        ...
        return JSONResponse({
            ...
            **_timing_locked(st),
            ...
            "has_aux_depth": st["aux_depth"] is not None,
